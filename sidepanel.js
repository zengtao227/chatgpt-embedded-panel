...
function renderModelStatus() {
  ...
  document.getElementById('model-requested').hidden = !lines.requested;
  document.getElementById('model-requested-value').textContent = lines.requested;
...
async function loadFrame({ cacheBust = false } = {}) {
  beginConnection();
  await runtimeMessage('chatgpt-panel.enable-policy');
...
async function openCompanion() {
  ...
  await runtimeMessage('chatgpt-panel.open-companion');
  setStatus('');
}

async function refreshPageState() {
  const payload = await runtimeMessage('webmcp.target-status');
...
stop.addEventListener('click', () => {
  void runtimeMessage('webmcp.task-stop')
    .then(async () => {
      setStatus('');
...
window.addEventListener('pagehide', () => {
  void chrome.runtime.sendMessage({ type: 'chatgpt-panel.closed' }).catch(() => {});
});
