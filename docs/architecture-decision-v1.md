...
- **`inspect_page` is viewport-first.** `MAX_ELEMENTS` stays 80; controls intersecting the viewport come first (document order), the rest fill remaining slots; at most 1500 candidates are examined. The result gains `viewport` (top-level scroll position and page size). The `text` field follows the viewport too (2026-09-21 addition after a live check on a long feed): text nodes from half a screen above to one screen below the viewport, in document order, at most 6000 characters and only what is on screen if the window holds more; `textScope` is `viewport`, or `page` when the page gives no usable geometry and the whole-page text is used. No cursor pagination.
- **A ref now stands for an element and what it was when inspected** (role, name, link target; editable controls without their live text). A reused DOM node that shows something else gets a new ref and the old ref keeps failing with `STALE_REF`, so windowed lists cannot redirect an old ref to a new row. Form controls (inputs, checkboxes, selects, editors) are identified by their **label**, not their live content, so filling does not invalidate them but a recycled row with another label does. At the start of every tool call, refs whose element was removed or no longer matches are **retired permanently**: a node that goes A, B, A does not revive the old ref (found by the Codex review of `ce98033`, fixed after). Residual limit: a node that changes and changes back entirely between two tool calls, or a recycled row with an identical label, cannot be told apart.
- **Not added in A4:** generic/page-wide keyboard, generic mouse, raw JavaScript, `chrome.debugger`, a shared executor package (the two `target-executor.js` copies stay in step by hand), or a higher `MAX_ELEMENTS`. The later element-scoped Browser Keyboard is governed by A5 below.
...
### A5 — 2026-09-23, Browser tool contract: six tools become seven (`keyboard`)

Evidence: `fill` is the preferred deterministic whole-value write, but real editors sometimes require caret/selection/deletion semantics for correcting only part of existing text or for edit surfaces that react to keyboard-style input state. Replacing `fill`, `select`, `click`, or `scroll` with keyboard simulation would make normal actions less deterministic, so all six existing semantic tools remain.

Decision:
- Add one stable **element-scoped** `keyboard({ ref, actions })` tool. `ref` must identify a previously inspected editable input, textarea, or contenteditable textbox; there is no page-wide keyboard target.
- `actions` is a bounded sequence (1–64) of either `{type:"text", text}` or `{type:"key", key, modifiers?, repeat?}`. This wire shape is intentionally broad enough that future key-policy changes do not require an eighth tool or a protocol redesign.
- Current execution policy supports text insertion, Backspace/Delete, caret movement, Home/End, Shift selection, Meta/Ctrl+A, and bounded word/line movement/deletion. `Enter`, `NumpadEnter`, and `Tab` fail closed with `CONFIRMATION_REQUIRED`; unsupported shortcuts return `KEY_NOT_SUPPORTED`.
- Keyboard is a precision-editing complement, not a replacement: `fill` remains the normal text write; `select` remains deterministic native-option selection; `click` retains commit-like action gating; `scroll` remains deterministic viewport/container navigation.
- Still not added: raw/page-wide keyboard injection, generic mouse, raw JavaScript, or `chrome.debugger`.
- ChatGPT and DeepSeek keep `browser-client.js` and `target-executor.js` byte-identical. Prism Web reuses the same two core files while keeping its owner-explicit target attachment UX.
